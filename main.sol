...
contract Ngo{
    ...
    event getAllCampaignsAndNgos_event(campaign[] campaigns);
    ...
    function getAllCampaignsAndNgos () public {
        emit getAllCampaignsAndNgos_event(Campaigns);
    }
    ...
    function _addCampaign(string calldata _name, string calldata _motto , uint  _amount) public {
        campaign memory new_camp = campaign(_name, _motto, _amount, 0, ngo_Users[msg.sender].name);
...
contract Individual{
    ...
    function _login (string calldata _userName, string calldata _password) public{
        if(keccak256(abi.encodePacked(Password[User_Ids[_userName]]))==keccak256(abi.encodePacked(_password))){
            emit _login_event(true, ind_Users[User_Ids[_userName]].name, ind_Users[User_Ids[_userName]].contact, ind_Users[User_Ids[_userName]].email, ind_Users[User_Ids[_userName]].Address);
        }
        else {
            emit _login_event(false, "","","","");
        }
    }
}
...
contract Corporation{
    ...
    function _login (string calldata _userName, string calldata  _password) public{
        if(keccak256(abi.encodePacked(Password[User_Ids[_userName]]))==keccak256(abi.encodePacked(_password))){
            emit _login_event(true, corp_Users[User_Ids[_userName]].name, corp_Users[User_Ids[_userName]].contact, corp_Users[User_Ids[_userName]].email, corp_Users[User_Ids[_userName]].Address);
        }
        else {
            emit _login_event(false, "","","","");
        }
